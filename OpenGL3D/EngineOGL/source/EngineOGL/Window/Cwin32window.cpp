...
Owindow::Owindow()
{
	...
	m_handle =  CreateWindowEx(NULL,MAKEINTATOM(classId), L"TEST|OpenGL", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, rc.right - rc.left, rc.bottom - rc.top, NULL, NULL, NULL, NULL);
	...
	wglChoosePixelFormatARB(hDC, pixeFormatAttributes, nullptr, 1, &pixelFormat, &numFormats);

	assert(numFormats);
	PIXELFORMATDESCRIPTOR pixelFormatDes = {};
	DescribePixelFormat(hDC, pixelFormat, sizeof(PIXELFORMATDESCRIPTOR), &pixelFormatDes);
	SetPixelFormat(hDC, pixelFormat, &pixelFormatDes);
	...
	m_context = wglCreateContextAttribsARB(hDC, 0, openGLAtts);
	assert(m_context);
